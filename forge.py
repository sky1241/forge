...
{sentinel}
# Generated by `forge --install-hook` from the venv at:
#   {python_path}
# Remove with `forge --uninstall-hook`, or edit freely — the uninstall
# command checks for the sentinel above and will refuse to remove a
# manually-edited hook.
#
...
#
# Why the absolute python path: git invokes hooks with a stripped PATH
# that doesn't include the user's venv. `forge` (the entry point script)
# lives in the venv's bin/ — not findable by `command -v forge` here.
# We embed the python path captured at install-time instead. If you
# move the venv, re-run `forge --install-hook --force`.
set -e
exec "{python_path}" -m forge --fast-deep
"""
